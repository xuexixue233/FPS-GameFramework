...
namespace FPS
{
    ...
    public class CameraShakeProfile
    {
        ...
        /// current state
        /// </summary>
        public FPS.CameraShakeState State
        {
            get
            {
                if (fadeTime < 1 && sustain && fadeInDuration > 0)
                    return FPS.CameraShakeState.FadingIn;

                else if (!sustain && fadeTime > 0)
                    return FPS.CameraShakeState.FadingOut;

                else if (fadeTime > 0 || sustain)
                    return FPS.CameraShakeState.Sustained;

                else
                    return FPS.CameraShakeState.Inactive;
            }
        }
    }
